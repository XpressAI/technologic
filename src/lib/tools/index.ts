...
function calculatorTool(): ToolSpec {
    ...
    const fn = async (toolCall: ToolCall, api) => {
        return api.toolOutput(
            api.toolName,
            eval(toolCall.calculation),
            undefined,
...
function encyclopediaTool(baseURL: string, vectorSpaceId: string, token: string): ToolSpec {
    ...
    const fn = async (toolCall: ToolCall, api) => {
        const baseURL = 'https://api.vecto.ai';
        const vectorSpaceId = '28325';
        const token = 'f9b71e54-8535-4f05-b6e7-d4d538784c6f';
        const query = toolCall.topic
