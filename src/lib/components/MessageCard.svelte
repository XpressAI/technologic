<script>
  ...
  $: contentItems = isStreaming
    ? msg.content + "<span class='streaming-message' />"
    : msg.content;
	...
	const nextThread = () => {
		dispatch('nextThread', msg);
	};
	const prevThread = () => {
		dispatch('prevThread', msg);
	};
	const regenerate = () => {
		dispatch('regenerate', msg);
	};
	const merge = () => {
		dispatch('merge', msg);
	};
    ...
    {/if}
		...
		<div class="flex items-center justify-between py-2">
			{#if alternativesCount > 1}
				<div>
					<button
						class="p-[0.5rem] w-0.5 h-0.5 btn-icon btn-icon-sm variant-soft-tertiary"
						on:click={prevThread}
