<script lang="ts">
	...
	$: {
		...
		const depths = Object.fromEntries(
			Object.keys($currentConversation?.messages).map((messageId) => {
				let depth = 0;
...
<div class="container w-full h-full flex p-4 gap-4">
		...
		<div class="relative overflow-y-auto flex-grow" style={`min-width: ${graphWidth}px; min-height: ${graphHeight}px`}>
			...
			>
				{#each $currentConversation?.graph as link}
					{#if link.from}
						<path
								...
								fill="none"
								class="{inCurrentThread(link.from) && inCurrentThread(link.to)
							? 'text-primary-500-400-token'
							: 'text-surface-300-600-token'} drop-shadow"
						/>
					{/if}
				{/each}
				{#each Object.entries(messagePositions) as [messageId, position]}
					...
					<g
							class="cursor-pointer {inCurrentThread(messageId)
					...
					>
						...
						{#if msg.message.role === 'user'}
							<g
									...
									transform="translate({position.x - 10}, {position.y - 10}) scale(0.8)"
									class="text-primary-700-200-token"
							>
						...
						{#if msg.message.role === 'assistant'}
							<g
									...
									stroke-linecap="round"
									stroke-linejoin="round"
									transform="translate({position.x - 9}, {position.y - 9}) scale(0.8)"
									class="text-primary-700-200-token"
							>
	...
	<div
		...
		style="width: max(36ch, {maxWidth * xOffset + xBase * 2}px)"
	>
		<h2>&nbsp;</h2>
		<div class="card variant-glass-primary p-2 overflow-hidden shadow">
			{#if hoveredMessageId}
